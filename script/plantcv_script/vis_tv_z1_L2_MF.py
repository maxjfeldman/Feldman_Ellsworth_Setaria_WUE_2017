...
def main():
  # Get options
  args = options()
  path_mask = '/home/mfeldman/tester/mask/mask_brass_tv_z1_L2.png'
  ...
  # This is a a hack until I can make a mask for this channel
  #masked_image = np.copy(img)
  
  # We can do a pretty good job of identifying the plant from the b channel
  device, b = pcv.rgb2gray_lab(masked_image, 'b', device, args.debug)
  b_thresh = cv2.inRange(b, 150, 255)
  
  # Lets blur the result a bit to get rid of unwanted noise
  b_blur = cv2.medianBlur(b_thresh,5)
  ...
  plant_shape = cv2.bitwise_and(a_blur, b_blur)
